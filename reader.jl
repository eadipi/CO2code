...
function plotSHEfficiency(file)
  z = read(file["z"]) * 1e3
  effic = read(file["efficSH"]) * 100
  return plot(scatter(x=z, y=effic), merge(lout_general, lout_effic))
end
...
function exportSHEfficiency(file)
  z = read(file["z"]) * 1e3
  effic = read(file["efficSH"]) * 100
  DB_Name = read(file["inp/DB_Name"]) * "-exports"
  checkpath(DB_Name)
  writedlm(DB_Name * "/efficSH.txt", [z;; effic])
  return nothing
end
...
@kwdef struct SimData
  ...
  plotSHEffic::Function = () -> plotSHEfficiency(file)
  ...
  exportSHEffic::Function = () -> exportSHEfficiency(file)
