...
function runcalc()
  ...
  A_komp::COdifferentialEqInputs = COdifferentialEqInputs(Aop=Aop, ATHz=ATHz, ASH=ASH)
  ...
  for ii in eachindex(z)[2:end]
    ...
    efficSH[ii] = sum(abs.(A_komp.ASH) .^ 2 ) / pF * npSH
    NcArray[ii] = 0
  ...
  DataBaseEnder(FID, z, t, nu, effic, NcArray, inputs, zsave, gamma, efficSH)
  close(FID)
